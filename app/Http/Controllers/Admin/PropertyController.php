...
class PropertyController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $property->pincode = $geo_id->pincode_id;
    ...
    public function updateDetails(Request $request, $id)
    {
        ...
        if ($property) {
            ...
            $checked_floors = isset($request->floor) ? $request->floor : [];
            for ($f = 0; $f < (int) $request->no_of_floors; $f++) {
                $floor_id = isset($request->floor_id[$f]) ? $request->floor_id[$f] : null;
                ...
                if (isset($request->nth_unit[$f])) {
                    if ((int) $request->nth_unit[$f] > 1) {
                        for ($u = 0; $u < (int) $request->nth_unit[$f]; $u++) {
                            ...
                            $unit_id = isset($request->$unit_id_key[$u]) ? (int) $request->$unit_id_key[$u] : '';
                            ...
                            $unit = FloorUnitMap::updateOrCreate(
                                ...
                                [
                                    ...
                                    'unit_brand_id' => isset($request->$unit_sub_category_key[$u]) && !empty($request->$unit_sub_category_key[$u]) ? $unit_brand_id : 0,
                                    'brand_name' => isset($request->$unit_sub_category_key[$u]) && !empty($request->$unit_sub_category_key[$u]) ? $uu_brand_name : '',
                                ],
                            );
                        ...
                        $floor_unit_id = !empty($floor_id) ? $floor_unit->id : null;
                        $floor_brand_id = null;
                        $floor_brand_name = '';
                        if (isset($request->floor_brand[$f])) {
                            if (is_numeric($request->floor_brand[$f])) {
                        ...
                        $unit = FloorUnitMap::updateOrCreate(
                            ...
                            [
                                ...
                                // 'merge_parent_unit_id'  => NULL,
                                'floor_unit_sub_cat_id' => 0,
                                'unit_brand_id' => $floor_brand_id,
                                'brand_name' => $floor_brand_name,
                            ],
                        );
                    }
                } else {
    ...
    public function check_gis_id(Request $request)
    {
        ...
        if ($geo_id) {
            ...
            // else {
            //     return response()->json(['status' => false, 'message' => ''], 422);
            // }
        } else {
    ...
    public function reportPropertyDetails(Request $request, $id)
    {
        ...
        return view('admin.pages.property.report_property_details', get_defined_vars());
    }
}
