...
class AmenitiesController extends Controller
{
    ...
    public function add_amminities_post(Request $request)
    {
        
        foreach($request->secondary_feature as $key=>$secondary_feature){
            if(isset($request->$secondary_feature)){
                foreach($request->$secondary_feature as $option){
                    $exist_amenity_map = Ammenitity::where('property_id', $request->property_id)->where('secondary_features_id', $secondary_feature)->
                    where('secondary_features_options_id', $option)->first();
                    if($exist_amenity_map)
                    {
                        $exist_amenity_map->property_id = $request->property_id;	
                        $exist_amenity_map->secondary_features_id = isset($secondary_feature) ? $secondary_feature : 0; 
                        $exist_amenity_map->secondary_features_options_id = isset($option) ? $option : 0;
                        $exist_amenity_map->save();   
                    }
                    else
                    {
                        $amenity_map  = new Ammenitity;
                        $amenity_map->property_id = $request->property_id;	
                        $amenity_map->secondary_features_id = $secondary_feature; 
                        $amenity_map->secondary_features_options_id = isset($option) ? $option :0;
                        $amenity_map->save();
                    }
                }
            }
        ...
        if($request->ajax()){
            return response()->json(['status'=> true, 'message' => 'Successfully Submitted.'],200);
        }
